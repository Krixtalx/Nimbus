...
#define NOTIFY_PROGRESS_WIDTH			250.f		// Progress bar fixed width
...
private:
	...
	float*										progressPerc = nullptr;

private:
	// Setters

	void setTitle(const char* format, const va_list args) {
		vsnprintf(this->title, sizeof(this->title), format, args);
	}

	void setContent(const char* format, const va_list args) {
		vsnprintf(this->content, sizeof(this->content), format, args);
	}

	void setButtonLabel(const char* format, const va_list args) {
...
public:
	...
	*/
	void setOnButtonPress(const std::function<void()>& onButtonPress) {
	 ...
	 * @param ... The arguments for the format string.
	*/
	void setButtonLabel(const char* format, ...) {
		NOTIFY_FORMAT(this->setButtonLabel, format);
	}

	void setProgressPerc(float& target) {
		this->progressPerc = &target;
	}
...
public:
		...
		if (!strlen(this->title)) {
			switch (this->type) {
				...
				case ImGuiToastType::Success:
					return LocaleStrings::getInstance()->getString(LOCALE_STRINGS::GENERIC_SUCCESS);
				case ImGuiToastType::Warning:
					return LocaleStrings::getInstance()->getString(LOCALE_STRINGS::GENERIC_WARNING);
				case ImGuiToastType::Error:
					return LocaleStrings::getInstance()->getString(LOCALE_STRINGS::GENERIC_ERROR);
				case ImGuiToastType::Info:
					return LocaleStrings::getInstance()->getString(LOCALE_STRINGS::GENERIC_INFO);
				default:
					return "";
			}
		}
	...
	ImGuiToastPhase getPhase() {
		...
		if (this->progressPerc != nullptr) {
			if (elapsed < NOTIFY_FADE_IN_OUT_TIME) {
				return ImGuiToastPhase::FadeIn;
			}
			return *this->progressPerc >= 1.0f ? ImGuiToastPhase::Expired : ImGuiToastPhase::Wait;
		}
	...
	*/
	std::function<void()> getOnButtonPress() {
...
public:
	 ...
	 */
	ImGuiToast(const ImGuiToastType type, const int dismissTime = NOTIFY_DEFAULT_DISMISS) {
		IM_ASSERT(type < ImGuiToastType::COUNT);
	 ...
	 */
	ImGuiToast(const ImGuiToastType type, const char* format, ...) : ImGuiToast(type) {
		NOTIFY_FORMAT(this->setContent, format);
	}
	 ...
	 * @param ... The variable arguments to be formatted according to the format string.
	 */
	ImGuiToast(const ImGuiToastType type, const int dismissTime, const char* format, ...) : ImGuiToast(type, dismissTime) {
		NOTIFY_FORMAT(this->setContent, format);
	}
	 ...
	 * @param ... The variable arguments to be formatted according to the format string.
	 */
	ImGuiToast(const ImGuiToastType type, const int dismissTime, const char* buttonLabel, const std::function<void()>& onButtonPress, const char* format, ...) : ImGuiToast(type, dismissTime) {
		NOTIFY_FORMAT(this->setContent, format);
	...
	ImGuiToast(const ImGuiToastType type, float& targetProgressPerc, const char* format, ...) : ImGuiToast(type) {
		NOTIFY_FORMAT(this->setContent, format);
		this->progressPerc = &targetProgressPerc;
	}
};
...
namespace ImGui {
	 ...
	 */
	inline void RemoveNotification(const int index) {
	...
	inline void RenderNotifications() {
		...
		for (size_t i = 0; i < notifications.size(); i++) {
			...
			std::string defaultTitle = currentToast->getDefaultTitle();
			...
			// Here we render the toast content
			{
				...
				if (!NOTIFY_NULL_OR_EMPTY(title)) {
					...
					if (!defaultTitle.empty()) {
						...
						Text("%s", defaultTitle.c_str()); // Render default title text (ImGuiToastType_Success -> "Success", etc...)
						wasTitleRendered = true;
					}
				...
				if (!NOTIFY_NULL_OR_EMPTY(content)) {
					...
					if (currentToast->getProgressPerc() != nullptr) {
						Text("%s %s", content, (static_cast<int>(ImGui::GetTime() / 0.1f) & 4) < 2 ? ICON_FA_CIRCLE : ICON_FA_CIRCLE_DOT); // Render content text
					}
					else {
				...
				// If a progress bar float is set
				if (currentToast->getProgressPerc() != nullptr) {

					float perc = *currentToast->getProgressPerc();

					ProgressBar(perc, ImVec2(NOTIFY_PROGRESS_WIDTH, 0));
