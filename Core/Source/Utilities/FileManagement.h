...
namespace FileManagement {
	...
	*	@brief
	*/
	void readTokens(const std::string& line, char delimiter, std::vector<std::string>& stringTokens, std::vector<float>& floatTokens);

	/**
	*	@brief Saves an image (array of bytes) in a system file, given the string filename.
	*/
	bool saveImage(const std::string& filename, std::vector<uint8_t>* image, unsigned int width, unsigned int height);
};
...
inline bool FileManagement::saveImage(const std::string& filename, std::vector<uint8_t>* image, const unsigned int width, const unsigned int height) {
