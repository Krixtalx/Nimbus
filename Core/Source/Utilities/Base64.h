...
public:
	...
	static bool Encode(const char *input, size_t input_length, char *out, const size_t out_length) {
		int i = 0, j = 0;
		char *out_begin = out;
		unsigned char a3[3];
		unsigned char a4[4];
	...
	*/
	static int DecodedLength(const char *in, const size_t in_length) {
		int numEq = 0;
	...
	static int EncodedLength(const size_t length) {
...
private:
	...
	static unsigned char b64_lookup(const unsigned char c) {
