...
static const float GRADIENT_BAR_WIDGET_HEIGHT = 15;
static const float GRADIENT_BAR_EDITOR_HEIGHT = 20;
...
ImGradient::ImGradient(const ImGradient& o)
{
    for (const auto mark : o.m_marks)
    {
        addMark(mark->position, mark->color[0]);
    }
}
...
std::vector<vec4> ImGradient::getColorArray() const
{
    std::vector<vec4> colors(256);
    for(int i = 0; i < 256; ++i)
    {
        colors[i] = vec4(
            m_cachedValues[i * 3 + 0],
            m_cachedValues[i * 3 + 1],
            m_cachedValues[i * 3 + 2],
            1.0f);
    }
    return colors;
}
...
namespace ImGui
{
                                  ...
                                  const float height)
    {
        ...
        for(auto markIt = gradient->getMarks().begin(); markIt != gradient->getMarks().end(); ++markIt )
        {
            ...
            draw_list->AddRectFilled(ImVec2(to - 6, barBottom),
                                     ...
                                     IM_COL32(100, 100, 100, 255), 0.0f, ImDrawFlags_None);
            ...
            if(selectedMark == mark)
            {
                ...
                draw_list->AddRect(ImVec2(to - 5, bar_pos.y + (height + 1)),
                                   ImVec2(to + 5, bar_pos.y + (height + 11)),
                                   IM_COL32(0, 255, 0, 255), 0.0f, ImDrawFlags_None);
            }
                        ...
                        const unsigned maxMarkCount)
    {
        ...
        if(ImGui::IsItemHovered() && ImGui::IsMouseClicked(0) && gradient->getMarks().size() < maxMarkCount)
        {
        ...
        if(selectedMark)
        {
            // bool colorModified = ImGui::ColorPicker3("###imgui_color_gradient_picker", selectedMark->color);
            bool colorModified = ImGui::ColorEdit3("###imgui_color_gradient_picker", selectedMark->color, ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_Float);
