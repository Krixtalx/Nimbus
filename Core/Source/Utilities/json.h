...
namespace json
{
	...
	public:
		 ...
		 */
		inline jobject(const bool array = false)
			: array_flag(array) 
			{ }
		...
		jobject& operator+=(const kvp& other)
		{
			...
			if(this->array_flag && !other.first.empty()) throw json::parsing_error("Array cannot have key");
			if(!this->array_flag && other.first.empty()) throw json::parsing_error("Missing key");
